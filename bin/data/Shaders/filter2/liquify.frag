...
uniform float time;
uniform float vol;
uniform float midi_effect2;
uniform float midi_effect3;
...
void main(){
	...
	vec2 shift;
	shift.x = cnoise( vec3(pos * 0.02, time * midi_time + 20.0) )* vol * 0.4 * midi_effect2;
	shift.y = cnoise( vec3(pos * 0.02, time * midi_time + 20.0) )* vol * 0.4 * midi_effect3;
	pos += shift * midi_size * 0.1;
